package com.sulake.habbo.navigator.roomsettings
{
   ...
   public class RoomSettingsCtrl
   {
      ...
      private var var_1805:ITextWindow;
      ...
      private var var_1192:ICheckBoxWindow;
      ...
      private function populateForm() : void
      {
         ...
         var_1193.setText(_loc1_.description);
         var_993.setText("");
         var_1190.setText("");
         ...
         changePasswordField(_loc1_.doorMode == RoomSettingsData.const_110);
         Logger.log("CATEGORY ID: " + _loc1_.categoryId);
         setCategorySelection(_loc1_.categoryId);
         refreshMaxVisitors(_loc1_);
         setTag(var_903,_loc1_.tags[0]);
         setTag(var_905,_loc1_.tags[1]);
         if(var_1191)
         {
            if(_loc1_.allowPets)
            {
      ...
      private function onConfirmRoomDelete(param1:WindowMouseEvent, param2:int) : void
      {
         ...
         close();
         var_399.roomSettingsRefreshNeeded();
         if(_navigator.data.guestRoomSearchResults != null)
         {
            _loc3_ = _navigator.data.guestRoomSearchResults;
            _navigator.mainViewCtrl.startSearch(_navigator.tabs.getSelected().id,_loc3_.searchType,_loc3_.include);
         }
      ...
      {
         Logger.log("[RoomSettingsCtrl.onDeleteButtonClick] " + var_400);
         _navigator.registerParameter("navigator.roomsettings.deleteroom.confirm.message","room_name",var_80.name);
         var _loc2_:ConfirmDialogView = new ConfirmDialogView(IFrameWindow(_navigator.getXmlWindow("ros_room_delete_confirm")),this,onConfirmRoomDelete,[var_400]);
         _loc2_.show();
      }
      ...
      public function refresh(param1:IWindowContainer) : void
      {
         ...
         var_1461.visible = _inRoom;
         var_1459.visible = _inRoom;
         var_1805.visible = var_476;
         var_1806.visible = !var_476;
         if(var_476)
         {
            var_994.visible = true;
            var_1188.visible = false;
            refreshFlatControllers();
            var_994.height = Util.getLowestPoint(this.var_994) + 4;
         }
         else
         {
            populateForm();
            var_990.visible = true;
            var_1188.visible = true;
            var_1189.visible = _inRoom;
            _navigator.thumbRenderer.refreshThumbnail(IWindowContainer(var_1189.findChildByName("picframe")),_navigator.data.enteredGuestRoom.thumbnail,false);
            var_990.height = Util.getLowestPoint(this.var_990) + 4;
         }
         var_1460.text = !!var_476 ? "${navigator.roomsettings.tobasicsettings}" : "${navigator.roomsettings.toadvancedsettings}";
         Util.moveChildrenToColumn(_window,[var_1461.name,var_1189.name,var_990.name,var_994.name,var_1188.name,var_1459.name],0,0);
      ...
      {
         this.var_400 = param1;
         _navigator.send(new GetRoomSettingsMessageComposer(var_400));
         if(var_991 != null)
         {
            var_991.visible = false;
         }
      }
      ...
      private function onSaveButtonClick(param1:WindowMouseEvent) : void
      {
         ...
         _loc2_.description = var_1193.getText();
         ...
         switch(_loc4_.name)
         {
            ...
            case "doormode_password":
               ...
               break;
            default:
               _loc2_.doorMode = RoomSettingsFlatInfo.const_375;
         }
         if(_loc2_.doorMode == RoomSettingsFlatInfo.const_110)
         {
            _loc8_ = var_993.getText();
            _loc9_ = var_1190.getText();
            if(_loc8_ != _loc9_)
            {
               this.var_993.clearErrors();
               this.var_1190.displayError("${navigator.roomsettings.invalidconfirm}");
               return;
            }
            if(_loc8_ != "")
            {
         ...
         _loc2_.allowWalkThrough = var_1192.isSelected;
         _loc2_.tags = new Array();
         addTag(var_903,_loc2_.tags);
         addTag(var_905,_loc2_.tags);
         this.clearErrors();
         this.var_992 = _loc2_.roomId;
      ...
      private function onRemoveAllFlatCtrlsClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         _navigator.send(new RemoveAllRightsMessageComposer(this.var_400));
      }
      
      public function onRoomSettingsSaved(param1:int) : void
      {
         if(param1 != this.var_400 || var_992 < 1)
         {
            return;
         }
         close();
      ...
      public function onRoomSettingsSaveError(param1:int, param2:int, param3:String) : void
      {
         ...
         var_992 = 0;
         if(param2 == RoomSettingsSaveErrorMessageParser.const_1399)
         {
         ...
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1400)
         {
            setTagError(this.var_903,param3,"${navigator.roomsettings.unacceptablewords}");
            setTagError(this.var_905,param3,"${navigator.roomsettings.unacceptablewords}");
         }
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1380)
         {
            setTagError(this.var_903,param3,"${navigator.roomsettings.nonuserchoosabletag}");
            setTagError(this.var_905,param3,"${navigator.roomsettings.nonuserchoosabletag}");
         }
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1296)
         {
            this.var_993.displayError("${navigator.roomsettings.passwordismandatory}");
         }
         else
         {
      ...
      private function prepareWindow(param1:IWindowContainer) : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_633,onDoorModePasswordSelect);
         _loc2_.addEventListener(WindowEvent.const_898,onDoorModePasswordUnselect);
         ...
         this.var_1193 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("description")),255);
         this.var_903 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("tag1")),30);
         this.var_905 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("tag2")),30);
         this.var_993 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("password")),30);
         this.var_1190 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("password_confirm")),30);
         ...
         this.var_1192 = ICheckBoxWindow(_window.findChildByName("allow_walk_through_checkbox"));
         var_1461 = IWindowContainer(_window.findChildByName("header_container"));
         var_1189 = IWindowContainer(_window.findChildByName("thumbnail_container"));
         var_990 = IWindowContainer(_window.findChildByName("basic_settings_container"));
         var_994 = IWindowContainer(_window.findChildByName("advanced_settings_container"));
         var_1459 = IWindowContainer(_window.findChildByName("switch_view_container"));
         var_1188 = IWindowContainer(_window.findChildByName("footer_container"));
         var_258 = IWindowContainer(_window.findChildByName("flat_controllers_container"));
         var_991 = IWindowContainer(_window.findChildByName("password_container"));
         var_1806 = ITextWindow(_window.findChildByName("basic_caption"));
         var_1805 = ITextWindow(_window.findChildByName("advanced_caption"));
         var_1460 = ITextWindow(_window.findChildByName("switch_view_text"));
         var_1460.procedure = onSwitchView;
         var_991.visible = false;
         if(!_inRoom)
         {
