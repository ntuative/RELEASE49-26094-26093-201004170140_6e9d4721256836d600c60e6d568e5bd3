package com.sulake.habbo.notifications
{
   ...
   public class HabboNotificationItemView implements IUpdateReceiver
   {
      ...
      private static const const_352:int = 0;
      
      private static const const_353:Number = 50;
      
      public static const const_597:int = 70;
      
      private static const const_799:int = 2;
      
      public static const const_461:int = 4;
      ...
      public function HabboNotificationItemView(param1:IAsset, param2:IHabboWindowManager, param3:Map, param4:Map, param5:IHabboToolbar)
      {
         ...
         if(_loc7_ != null)
         {
            var_1371 = 0 - _loc7_.rectangle.bottom;
         }
         else
         {
            var_1371 = 15;
      ...
      private function setNotificationText(param1:String) : void
      {
         ...
         _window.height = 0;
         _loc2_.text = param1;
         _loc2_.height = _loc2_.textHeight + var_1371;
      ...
      public function update(param1:uint) : void
      {
         ...
         switch(var_32)
         {
            ...
            case const_368:
               var_934 += param1;
               _loc2_ = Number(var_934) / Number(var_446["time_fade_in"]);
               if(var_934 > int(var_446["time_fade_in"]))
               {
                  startDisplay();
               }
               adjustBlend(_loc2_);
            ...
            case const_799:
               var_1096 += param1;
               if(var_1096 > int(var_446["time_display"]) && !var_1370)
               {
                  startFadeOut();
               }
               break;
            case const_798:
               var_933 += param1;
               _loc3_ = 1 - Number(var_933) / Number(var_446["time_fade_out"]);
               adjustBlend(_loc3_);
               if(var_933 > int(var_446["time_fade_in"]))
               {
                  startIdling();
               }
         }
      ...
      public function reposition(param1:int = -1) : void
      {
         ...
         var _loc3_:int = var_193.orientation == "TOP" ? int(const_353) : 0;
         var _loc4_:int = var_193.orientation == "BOTTOM" ? int(_loc2_.height - const_353) : int(_loc2_.height);
         var _loc5_:int = var_193.orientation == "LEFT" ? int(const_353) : 0;
         var _loc6_:int = var_193.orientation == "RIGHT" ? int(_loc2_.width - const_353) : int(_loc2_.width);
         if(false)
         {
            if(false)
            {
               _window.x = _loc5_ + const_461;
            }
            else
            {
               _window.x = _loc6_ - 0 - const_461;
            }
         }
         else if(false)
         {
         ...
         if(false)
         {
            if(false)
            {
               _window.y = _loc3_ + var_628;
            }
            else
            {
               _window.y = _loc4_ - 0 - var_628;
            }
         }
         else if(false)
         {
