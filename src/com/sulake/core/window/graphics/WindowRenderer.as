package com.sulake.core.window.graphics
{
   ...
   public class WindowRenderer implements IWindowRenderer
   {
      ...
      public function render() : void
      {
         ...
         while(_loc4_ < _loc2_)
         {
            _loc1_ = var_623.pop() as WindowController;
            if(!_loc1_.disposed)
            {
               _loc3_ = getWindowRendererItem(_loc1_);
               var_317.x = _loc3_.dirty.x;
               var_317.y = _loc3_.dirty.y;
               var_317.width = _loc3_.dirty.width;
               var_317.height = _loc3_.dirty.height;
               renderWindowBranch(_loc1_,_loc3_,var_317);
            }
            _loc4_++;
         }
      }
      ...
      {
         param1.removeEventListener(WindowDisposeEvent.const_530,windowDisposedCallback);
         var _loc2_:WindowRendererItem = var_316[param1] as WindowRendererItem;
         if(_loc2_ != null)
         {
            _loc2_.dispose();
      ...
      private function renderWindowBranch(param1:WindowController, param2:WindowRendererItem, param3:Rectangle) : void
      {
         ...
         if(param1.visible)
         {
            if(getDrawLocationAndClipRegion(param1,param3,var_1580,var_1579))
            {
               if(param2.render(param1,var_1580,var_1579))
               {
      ...
      public function registerRenderable(param1:IWindow) : void
      {
         ...
         if(_loc2_ == null)
         {
            _loc2_ = new WindowRendererItem(this,_drawBufferAllocator,var_752);
            var_316[param1] = _loc2_;
            param1.addEventListener(WindowDisposeEvent.const_530,windowDisposedCallback);
         }
      }
      ...
      public function addToRenderQueue(param1:IWindow, param2:Rectangle, param3:uint) : void
      {
         ...
         if(getWindowRendererItem(param1).invalidate(param1,param2,param3))
         {
            ...
            var_317.x = param2.x - param1.x;
            var_317.y = param2.y - param1.y;
            var_317.width = param2.width;
            var_317.height = param2.height;
            if(param1.testParamFlag(WindowParam.const_34))
            {
               while(param1.testParamFlag(WindowParam.const_34))
               {
                  ...
                  var_317.offset(param1.x,param1.y);
                  param1 = _loc4_;
               }
            }
            getWindowRendererItem(param1).invalidate(param1,var_317,WindowRedrawFlag.const_243);
            if(var_623.indexOf(param1) == -1)
            {
               var_623.push(param1);
            }
         }
