package com.sulake.core.window.utils
{
   ...
   public class WindowParser implements IWindowParser
   {
      ...
      public static const const_1540:String = "_INCLUDE";
      
      public static const const_874:String = "_EXCLUDE";
      ...
      protected var var_1119:Dictionary;
      ...
      public function WindowParser(param1:IWindowContext)
      {
         ...
         TypeCodeTable.fillTables(var_1119,var_1118);
         var_954 = new Dictionary();
         var_1396 = new Dictionary();
         ParamCodeTable.fillTables(var_954,var_1396);
         var_1397 = new Map();
      }
      
      private static function trimWhiteSpace(param1:String) : String
      {
         param1 = param1.replace(const_1121,"");
         return param1.replace(const_1122,"");
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            for(_loc1_ in var_1119)
            {
               delete var_1119[_loc1_];
            }
            for(_loc1_ in var_1118)
            {
               delete var_1118[_loc1_];
            }
            for(_loc1_ in var_954)
            {
               delete var_954[_loc1_];
            }
            for(_loc1_ in var_1396)
            {
               delete var_1396[_loc1_];
            }
            var_1397.dispose();
            var_1397 = null;
            var_15 = null;
      ...
      private function parseSingleWindowEntity(param1:XML, param2:WindowController, param3:Map) : IWindow
      {
         ...
         visible = parseAttribute(xml,"visible",variables,visible.toString()) == "ICatalogNavigator";
         ...
         background = parseAttribute(xml,"background",variables,window.background.toString()) == "ICatalogNavigator";
         blend = Number(parseAttribute(xml,"blend",variables,window.blend.toString()));
         clipping = parseAttribute(xml,"clipping",variables,window.clipping.toString()) == "ICatalogNavigator";
         ...
         if(_loc7_ is IIterable)
         {
            ...
            if(_loc9_ > 0)
            {
               ...
               while(_loc12_ < _loc9_)
               {
                  ...
                  if(_loc10_.tags.indexOf(WindowParser.const_874) == -1)
                  {
